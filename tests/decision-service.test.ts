...
import type { Scorer } from "../src/scoring";
const input = {
  decision: "Ship?",
  context: "Tests pass.",
  choices: [
...
test("automatic routing chooses a profile, applies global and profile policies, and shares one deadline", async () => {
  ...
  const result = await Effect.runPromise(createDecisionService({ config, score })({ input }));
    ...
    const score: Scorer = (args) => {
      return Effect.sync(() => {
        return {
          ...
          percentageSource: percentage === null ? "unavailable" : "provider-distribution",
          model: args.model.model,
          provider: args.model.provider,
          warnings: [],
        };
...
test("explicit profiles bypass routing and unknown profiles fail before provider I/O", async () => {
  ...
  expect((await Effect.runPromise(decide({ input, profileId: "safety" }))).routing.mode).toBe(
    "explicit",
  );

  expect(count).toBe(1);

  await expect(Effect.runPromise(decide({ input, profileId: "missing" }))).rejects.toThrow(
    "Unknown profile",
  );
...
test("one Effect deadline interrupts routing and scoring together", async () => {
  ...
  await Effect.runPromise(
    Effect.gen(function* () {
      const fiber = yield* Effect.forkChild(createDecisionService({ config, score })({ input }));
