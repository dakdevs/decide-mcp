...
test("npm tarball installs independently and exposes a working MCP executable", async () => {
  ...
  try {
    const packed = await exec("npm", ["pack", "--json", "--pack-destination", directory], {
    ...
    const files = manifest[0]!.files.map((file) => {
      return file.path;
    });
    ...
    const metadata = JSON.parse(
      await readFile(join(directory, "node_modules/decide-mcp/package.json"), "utf8"),
    );

    expect(metadata.private).not.toBe(true);

    expect((await exec(executable, ["--version"], { cwd: directory })).stdout.trim()).toBe(
      metadata.version,
    );

    expect((await exec(executable, ["--help"], { cwd: directory })).stdout).toContain(
      "MCP over stdio",
    );
