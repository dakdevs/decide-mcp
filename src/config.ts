...
const modelSchema = Schema.Struct({
  ...
  providerOptions: Schema.optionalKey(
...
export const providerSchema = Schema.Struct({
  ...
  baseURL: Schema.optionalKey(
    Schema.String.check(
      Schema.makeFilter((value) => URL.canParse(value) || "Invalid URL"),
    ),
  ),
...
export const configSchema = Schema.Struct({
  ...
  providers: Schema.Record(identifier, providerSchema).pipe(
    Schema.withDecodingDefaultKey(
      Effect.succeed({ gateway: { kind: "gateway" as const } }),
    ),
  ),
  profiles: Schema.Array(
    Schema.Struct({
      id: identifier.check(
        Schema.makeFilter((id) => id !== "default" || "default is reserved"),
      ),
  ...
  router: Schema.Struct({
    ...
    minimumProbability: Schema.Number.check(
  ...
  timeoutMs: Schema.Int.check(
    Schema.isBetween({ minimum: 1, maximum: 300000 }),
  ).pipe(Schema.withDecodingDefaultKey(Effect.succeed(30000))),
  ...
  Schema.makeFilter((config) => {
    ...
    if (new Set(ids).size !== ids.length) return "Profile IDs must be unique";
    for (const model of [
      config.model,
      config.router.model,
