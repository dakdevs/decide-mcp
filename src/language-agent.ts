...
export function runStructuredAgent<Input, Output>({
  input,
  model,
  modelConfig,
  config,
  ...
  input: Input;
  inputSchema: Schema.Codec<Input, unknown>;
  outputSchema: Schema.Codec<Output, unknown>;
  ...
  return Effect.gen(function* () {
    ...
    const generate = (options: LanguageModel.ProviderOptions) => {
      return Effect.tryPromise({
        try: (signal) => {
          return generateText({
            ...
            output: Output.object({
              schema: Schema.toStandardSchemaV1(Schema.toStandardJSONSchemaV1(outputSchema), {
        ...
        Effect.tap((result) => {
          return Ref.set(warnings, Boolean(result.warnings?.length)).pipe(
            Effect.andThen(
              Ref.set(usage, {
...
export const runLanguageAgent = ({
  ...
  return runStructuredAgent({
    ...
    Effect.map((result) => {
      return { estimates: result.output.choices, hasWarnings: result.hasWarnings };
    }),
  );
};
